...
contract CommunityProjectManagementSystem {
    struct Task {
        string taskName;
        string description;
        address assignedTo;
    ...
    struct Project {
        string projectName;
        string description;
        uint256[] taskIds; // Список ID задач
    }
    ...
    event TaskAdded(uint256 indexed projectId, uint256 indexed taskId, string taskName, string description);
    ...
    function addTask(uint256 projectId, string memory taskName, string memory description) public {
        ...
        emit TaskAdded(projectId, taskId, taskName, description);
    }
    ...
    function getProject(uint256 projectId) public view returns (string memory, string memory, Task[] memory) {
        ...
        Task[] memory taskArray = new Task[](project.taskIds.length);
        for (uint256 i = 0; i < project.taskIds.length; i++) {
            taskArray[i] = tasks[project.taskIds[i]];
        }
        return (project.projectName, project.description, taskArray);
    }
